...
const NewTaskForm = () => {
  ...
  const SubmitButton = () => {
    ...
    return (
      <button type="submit" className="mt-8 py-2 w-full rounded-md text-white bg-pink-400 hover:bg-pink-200 text-sm font-semibold shadow-sm disabled:bg-gray-400" disabled={pending} >Create</button>
    )
  }
  ...
  return (
    <div className="mt-10 mx-auto w-full max-w-sm">
      <form action={formAction}>
        ...
        <SubmitButton />
        { state.error && <div className="mt-2 text-red-500 text-sm">{state.error}</div> }
      </form>
    </div>
  )
}
